...
void sendpacket(char* hostName,char * message)
{
 ...
 /*sockfd=getSocketFd(nextdest.ifacename);*/
 ...
 if (addr.s_addr==nextdest.nexthop)
 {
  ...
  /*Need to implement this functionality later
  getMacfromarpcache();  */
  /*If Ip address present in the same LAN then try to get the mac address of the destination*/
  ...
  /*if (strcmp(hostName,"A")==0)
  {
   strncpy((char *)nexthop_mac,"00:00:0C:04:52:27",18);
  }
  else
  {
   strncpy((char *)nexthop_mac,"00:00:0C:04:52:38",18);
  }*/
 }
 else
 {
   ...
   else if (arppkt.op==ARP_RESPONSE)
   {
    if (isPacketForStation(arppkt.srcip))
    {
     ...
     if (index_pendpkt>-1)
     {
      ...
      /*index_mac=getMacAddressIndex(pendingPkts[index_pendpkt]->packet.srcip);*/
